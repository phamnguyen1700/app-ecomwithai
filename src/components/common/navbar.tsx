...
export default function Navbar() {
  ...
  useEffect(() => {//eslint-disable-line
    const updateCart = () => {//eslint-disable-line
      const cart = JSON.parse(localStorage.getItem(CART_STORAGE_KEY) || "[]")//eslint-disable-line  
      setCartItems(cart)//eslint-disable-line
      setCartCount(cart.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0))//eslint-disable-line
    }//eslint-disable-line
          ...
          <Popover>
            ...
            <PopoverContent className="w-80">
              ...
              <div className="grid gap-2">
                {cartItems.length === 0 ? (
                  <div className="text-center text-xs text-gray-400 py-6">Bạn chưa mua sản phẩm nào</div>
                ) : (
                  cartItems.map((item, idx) => (
                    <div key={idx} className="flex items-center border-b last:border-b-0 py-2">
                      <div className="w-14 h-14 flex-shrink-0 relative">
                        <Image src={item.image || '/assets/blank.png'} alt={item.name} fill className="object-cover rounded" />
                      </div>
                      <div className="flex-1 ml-3 flex flex-col justify-between h-full">
                        ...
                        <div className="text-[10px] text-gray-500 mt-1">Số lượng: {item.quantity}</div>
                      </div>
                      <div className="ml-auto flex flex-col justify-end h-full">
                        <span className="text-xs font-semibold text-right">{formatMoney(item.price)}</span>
                      </div>
                    </div>
                  ))
                )}
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  🔥Chúng tôi sẽ gửi những thông tin khuyến mãi mới nhất đến email của bạn!🔥
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                <Button className="w-full text-xs" type="submit">Đăng Nhập</Button>
              </DialogFooter>
              <p className="text-[10px] text-center text-gray-500">
                Bằng việc nhấn Đăng Nhập, bạn đồng ý với{' '}
                <Link href="#" className="underline">Điều khoản sử dụng</Link> và{' '}
                <Link href="#" className="underline">Chính sách bảo mật</Link>.
              </p>
            </DialogContent>
          </Dialog>
        </div>
